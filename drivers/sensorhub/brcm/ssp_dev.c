...
int send_panel_information(struct panel_bl_event_data *evdata){
	...
	int buf[2] = { evdata->brightness, evdata->aor_ratio };
	int iRet = 0;
...
static int panel_notifier_callback(struct notifier_block *self, unsigned long event, void *data){
	...
	if (event == PANEL_EVENT_BL_CHANGED) {
		...
		if((enum panel_notifier_event_copr_state)evt_data.state != PANEL_EVENT_COPR_ENABLED && (enum panel_notifier_event_copr_state)evt_data.state != PANEL_EVENT_COPR_DISABLED){
